...
Attitude * makeAttTako(InitI *ini, EphemData *ephem) {
  ...
  if ( (ITL=fopen(ini->TLname.c_str(),"r")) == NULL) {
    ...
    // Many of these variables below appear to be initializations of values referenced later.  Not sure if we can set them now.
      ...
      double mjdt = 0.0; //Profile Start MJD  *****What is this?  The start of the rocking profile? *****
      ...
      SurProf profile; //What is this?

      double lastend = 0.0; //I think I added this in as the last ending value, but I need to double check
    ...
    if(OAtt->mjd[i] >= org_stT && OAtt->mjd[i] <= org_enT) {
      ...
      */
      ...
      if(i>1){
        ...
        if(fabs((OAtt->mjd[i] - OAtt->mjd[i-1]) - ini->Resolution) > 0.000000001){
          ...
          << " minutes difference.\n  "
          << (fabs((OAtt->mjd[i]-OAtt->mjd[i-1])-ini->Resolution))*86400000000
          << " microseconds off from the expected resolution.\n";
