...
void orbitSimApp::run() {
  ...
  if(match_str((const char*) Input.c_str(), "FILE") == 1){
    ...
    if((match_str((const char*) Tlt.c_str(), "TAKO") == 1) || (match_str((const char*) Tlt.c_str(), "ASFLOWN") == 1)){
      ...
      //Rename initf.TLname to initf.timeline and make it a
      //reference to the timeline object generated by the parser
      timeline tl = timeline(Tml);
      initf.timeline = tl;
...
//    	 (match((const char*) EpF.c_str(), "^tlederive$")==1))){
  ...
  if (stat == 0){
    ...
    oBuf << "Timeline file or cmd   " << Tml << std::endl; 
