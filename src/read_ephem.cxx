/**
 * @file read_ephem.cc
 * @brief This file contains functions to read/calculate ephemerides and the
 * attitude calculation.
 * @author Giuseppe Romeo
 * @date Created:  Nov 15, 2005
 *
 * $Header: /nfs/slac/g/glast/ground/cvs/orbitSim/src/read_ephem.cxx,v 1.10
 * 2009/06/23 17:51:24 vernaleo Exp $
 */
...
Attitude*
deallocateAttitude(Attitude* att)
{
...
Attitude*
reallocateAttitude(int num, Attitude* att)
{
  att->mjd.resize(num);
  att->X.resize(num);
  att->Y.resize(num);
  ...
  double Resolution)
{
  ...
  { /* either our resolution or our file size isn't enough */
    osf.warn() << "Warning-- ephemeris does not have enough points to cover "
                  "time of interest\nfound "
               << ipts
               << " points, we will still try to do the best to go on!\n";
  }
  ...
  if (tmjd > StartTime)
  {
    ...
    eBufT << "\n"
          << __FILE__ << ":" << __LINE__
          << ", ERROR: Ephemeris file does not cover the entire time "
             "interval\nEphemeris file starts at "
          << tmjd << " while the interval of interest starts at " << StartTime
  ...
  while (okay)
  {
    ...
    {
      osf.err()
        << "Error, ephemeris file does not reach start of schedule, exiting\n";
      return (NULL);
    }
    tmjd = do_cal2mjd(tyear, tmonth, tday, thour, tmin, tsec);
  ...
  while (it < inum)
  { /* keep reading until we get to end of timespan */
    ...
    {
      osf.err()
        << "Error, ephemeris file does not reach end of schedule, exiting\n";
      return (NULL);
    }
    ...
    if (ckflg == 2)
    {
      ...
      if (fabs(dt - Resolution) > 1.0e-10)
      {
        ...
        eBufT << "\n"
              << __FILE__ << ":" << __LINE__
              << ", ERROR: The Ephemeris file does not have the requested time "
...
/* CKS: The xyzll_eph routine was written to ingest the STK generated ephemeris
   for Swift. It is similiar to the yyyy_eph routine.  The differences include a
   comment line as the first line of data from STK and the lattitude and
   longitude will be used directly from the STK data.  The altitude will be
   calculated, but is not currently planned to be used.

   The time format is also different, Swift will use a UTCJ format: doy/year
   HH:MM:SS.ss. Swift ephem also includes the velocity RA and DEC of the
   satellite.
*/
  ...
  double Resolution)
{
  ...
  /*
   ...
   DOY/YYYY HH:Min:SS.s Satellite_X_pos Satellite_Y_pos Satellite_Z_pos Lat Long
   veocity_ra velocity_dec X, Y, Z are in  km, and Lat and Long (degrees) used,
   velocity Ra and Dec (degrees) and Altitude is calculated  */
  ...
  while (okay)
  {
    ...
    if (iret == EOF)
    {
      osf.err() << "xyzll_eph: Error, ephemeris file does not reach start of "
                   "schedule, exiting\n";
      return EPHNO;
    }
    ...
    {
      osf.err() << "xyzll_eph: Error, StartTime (" << StartTime
                << ") is prior to ephemeris data (" << (mjd - Resolution)
                << ").  Exiting.\n";
      return (EPHNO);
    }

    /* Check that the ephemeris resolution is the same as the TAKO resolution
     * (within 3 seconds */
    if ((rez_check) && (mjd != mjd1))
    {
      ...
      else
      {
        ...
        osf.err() << "xyzll_eph: Error, the ephemeris resolution (" << rez
                  << ") is NOT the same as the Tako Resolution (" << Resolution
                  << "). Exiting.\n"
                  << "The times compared were:  MJD= " << mjd1
                  << " and MJD1= " << mjd << "\n";
        return (EPHNO);
      }
    }
  ...
  { /* keep reading until we get to end of timespan */
    ...
    if (iret == EOF)
    {
      osf.err() << "xyzll_eph: Error, ephemeris file does not reach end of "
                   "schedule, exiting\n";
      return EPHNO;
    }
  ...
  double Resolution)
{
  ...
  double temp;
  char const* SatN = "GLAST";  // TLE satellite name is 5 characters
  char const* SatN2 = "FGRST"; // This is the name used in NORAD file
  ...
  {
    osf.err()
      << "Error: Satellite " << SatN << " or " << SatN2
      << " is NOT present in the TLE file\nPlease, check it and try again\n\n";
    return (EPHNO);
  }
    ...
    */
    ...
    while (it < inum)
    {
      ...
      if (pos.v[0] == 0.0 && pos.v[1] == 0.0 && pos.v[0] == 0.0)
        osf.warn() << "at Sat#=" << is << "X=" << pos.v[0] * xkmper
                   << ", Y=" << pos.v[1] * xkmper << ",Z=" << pos.v[2] * xkmper
                   << ", the position vector is null \n";
      // printf("%4d %02d %02d %02d %02d %05d %f %f %f %f\n",tz.yr, tz.mo,
      // tz.dy, tz.hr, tz.mn, tz.sc, pos.v[0]* xkmper, pos.v[1]* xkmper,
      // pos.v[2]* xkmper, Units); 1

      /*
            if(it < 0)
              printf("%4d %02d %02d %02d %02d %05d %f %f %f %f\n",tz.yr, tz.mo,
         tz.dy, tz.hr, tz.mn, tz.sc, pos.v[0]* xkmper, pos.v[1]* xkmper,
         pos.v[2]* xkmper, Units);
      */
  ...
  double TS)
{
  ...
  if (mode != 2)
  {
    ...
    if (mode == 1 && start < mjds)
    {
      ...
      double RaDec[2];
      TOAtt->ent = oas;
      osf.info(3) << "Calling MakeSurvey(" << start - res << ", " << mjde + res
                  << ", " << res << ", " << offset << "\n";
    ...
    if (mode == 1)
    {
      ...
      else
      {
        ...
        for (ii = j; ii < k; ii++)
        {
          ...
          osf.info(6) << "ii=" << ii << ", Zra=" << TOAtt->Zra[ii]
                      << ", Zdec=" << TOAtt->Zdec[ii] << "\n";
        }
  ...
  osf.info(3) << "\nLeaving MakeAtt with lpos[0]=" << lpos[0]
              << ", lpos[1]=" << lpos[1] << "\n\n\n";
...
void MakeAtt(double start, double mjde, double mjds, double pra,
             ...
             double res, EphemData *ephem, double *lpos, Attitude *OAtt, double
TS ) {
...
//  slewing must be calculated
  ...
  //  printf ("2) i=45 ==> mjd=%f, i=46 ==> mjd=%f\n", OAtt->mjd[45],
OAtt->mjd[46]); if(mode == 1) { if(flgS == 0){ double RaDec[2];// = NULL;
      MakeSurvey(mjds, mjde, res, offset, ephem, OAtt, RaDec, 1, TS);
      ...
      for(ii=j; ii<k; ii++){
        ...
        osf.info(3) <<"ii="<<ii<<", Zra="<<TOAtt->Zra[ii]<<",
Zdec="<<TOAtt->Zdec[ii]<<"\n";
      }
      lpos[0] = TOAtt->Zra[k-1];
      lpos[1] = TOAtt->Zdec[k-1];
  ...
  double TS)
{
  ...
  double RaDe[11];
  int i, inum, k;
  double sra, sdec, zra, zdec, yra, ydec, xra, xdec, sunx, suny, sunz, suna,
    sunxy;
      ...
      *******************************************************************************/
      ...
      for (jj = SSunPts; jj < SSunPts + TSunPts; jj++)
      {
        ...
        */
        ...
        GetPos(mjd, vSat, vNVelS, offset, RaDe);
        //	printf("==>%4d) jj=%d mjd=%f, zdecO=%f, offset=%f, ", k, jj,
        // mjd, RaDe[3], RAD2DEG*offset);
        GetPos(mjd, vSat, vNVelS, SunSurvOfs, RaDe);
  ...
  double TS)
{
  ...
  angularSep(
    pra * DEG2RAD, pdec * DEG2RAD, ra * DEG2RAD, dec * DEG2RAD, &theta);

  double dlon = (ra - pra) * DEG2RAD;
  double dlat = (dec - pdec) * DEG2RAD;
  double a = sin(dlat / 2.0) * sin(dlat / 2.0)
    + cos(pdec * DEG2RAD) * cos(dec * DEG2RAD) * sin(dlon / 2.0)
      * sin(dlon / 2.0);
  ...
  double duratS = GLAST_slew_estimate(
    raS, decS, pra * DEG2RAD, pdec * DEG2RAD, ra * DEG2RAD, dec * DEG2RAD);
  ...
  */
  ...
  if (inum != tm)
  {
    osf.warn() << "\nFound a discrepancy:\nmjds=" << mjds << ", start=" << start
               << ", res=" << res << "\n";
    osf.warn() << "in DoSlew, pra=" << pra << ", pdec=" << pdec << ", ra=" << ra
               << ", dec=" << dec << ", with inum=" << inum << ", tm=" << tm
               << ", dt=" << (mjds - start) * 1440.0 << ", thetad=" << thetad
               << ", slewR=" << slewR << ", k=" << k << "\n";
    osf.warn() << "With a slew rate of " << slewR
               << ", and an angular separation of " << thetad << ", there are "
               << tm << " points,\nangular increment is " << incrang
               << "\npra=" << pra << ", pdec=" << pdec << "\nra=" << ra
               << ", dec=" << dec << "\nresi1=" << resi1 << ",  resi2=" << resi2
               << ", fact=" << fact << "\n\n";
  }
  ...
  for (i = 0; i < tm; i++)
  {
    ...
    OAtt->Zra[k] = RaDe[2];
    OAtt->Zdec[k] = RaDe[3];
    // in_occ = -99999  indicates this is a maneuver so it is ignored in the
    // limb trace routine
    OAtt->in_occ[k] = -99999;
  ...
  double TS)
{
  ...
  for (i = j; i < inum + j; ++i)
  {
    ...
    /* For pointed observation, the rock angle is not used for calculating
     * attitude. In order to output the rock angle to the fits file, we need to
     * calculate the rock angle here. We need the satellite position vector and
     * attitude to get the rock angle. Note by definition, the rock angle is the
     * angle between zenith direction and direction of satellite pointing
...
/* CKS: The saa routine was formally called xtesaa. Since this was the method
   ...
   This routine determines if each ephemeris point is in or out of the SAA
   polynomial. A filename should be passed in which defines SAA polynomial
   (longitude/latitude pairs). In addition to an ephemeris point being in the
   SAA polynomial, the ephem point prior will also be included as in the SAA
   polynomial,because somewhere between the two points is when the actual SAA is
   entered (we're rounding by one resolution). The end of an SAA already rounds
   to next resolution (the value remains true for full resolution).
  ...
  Attitude* att)
{
  ...
  if (strcmp(filename, "NULL") != 0)
  {
    ...
    else
    {
      ...
      while (
        (fgets(line, lnsz, fptr) != NULL) && (num_lat < 50) && (num_lon < 50))
      {
        ...
        if (line[0] != '#')
        {
          ...
          else if (strncmp(line, "SAAPOLYLONG", 11) == 0)
          {
            ...
            if (lontable[num_lon] >= 180.0)
              lontable[num_lon] -= 360.0;
          }
          else
          {
            sscanf(line, "%lf %lf", &lontable[num_saa], &lattable[num_saa]);
            if (lontable[num_saa] >= 180.0)
              lontable[num_saa] -= 360.0;
            num_saa++;
            num_lat++;
            num_lon++;
          }
      ...
      /*  For the following algorithm to work, the last pair must be the same as
         the 1st. If the file doesn't do this (likely), then add the last pair.
       */

      if (num_saa > 0)
        if ((lontable[1] != lontable[num_saa])
          || (lattable[1] != lattable[num_saa]))
        {
          num_saa++;
  ...
  if (find_minmax(
        &lontable[0], &lattable[0], num_saa, &minLon, &maxLon, &minLat, &maxLat)
    != 0)
  {
    std::ostringstream eBufT;
    eBufT << "\n"
          << __FILE__ << ":" << __LINE__
          << "saa constraint could not find min and max saa (lat,lon)\n\n"
          << std::ends;
  ...
  if (calculate_slopes(
        &lontable[0], &lattable[0], num_saa, &slopes[0], &intercepts[0])
    != 0)
  {
    std::ostringstream eBufT;
    eBufT << "\n"
          << __FILE__ << ":" << __LINE__
          << "saa constraint could not find saa slopes/intercepts\n\n"
          << std::ends;
  ...
  /* Make sure the constraint encloses the SAA period. For the beginning of the
     ...
     resoulution time).
     The resulting SAA constraint period will have up to one resolution (one
     minute) padding at the beginning and end. */
  ...
  Attitude* OAtt)
{
  ...
  osf.info(4) << __FILE__ << ":" << __LINE__
              << ", called MakeSurvey with offset=" << offset << "\n\n";
  ...
  if (start < mjds)
  {
    ...
    osf.info(4) << __FILE__ << ":" << __LINE__
                << ", called DoSlew with offset=" << offset << "\n\n";
  }
  ...
  int k;
  osf.info(4) << __FILE__ << ":" << __LINE__
              << ", the first point after the slew is at k=" << (inum - inum1)
              << "\n\n";
  ...
  for (k = (inum - inum1); k < inum; k++)
  {
    ...
    if (k < inum - 2)
    {
      if (ephem->Z[k] <= 0.0 && ephem->Z[k + 1] >= 0.0)
      {
        ...
        /*
                if(ia == 1){
                  printf ("%4d) Transition taking place at
           %4d/%03d:%02d:%02d:%02d starting offset=%5.2f ending offset=%5.2f
           since z0=%f and z1=%f\n", k, yy, dy, hh, mm, ss, offset, -offset,
           ephem->Z[k], ephem->Z[k+1]); osf.info() << "i=0, OAtt->mjd[0]=" <<
  ...
  double res)
{
  ...
  while (ntr < 20)
  {
    ...
    angularSep(
      ira * DEG2RAD, idec * DEG2RAD, fra * DEG2RAD, fdec * DEG2RAD, &theta);
    ...
    *mjds = mjdi + slewT;
    ...
    double duratS = GLAST_slew_estimate(
      raS, decS, ira * DEG2RAD, idec * DEG2RAD, fra * DEG2RAD, fdec * DEG2RAD);
    ...
    *mjds = mjdi + duratS;
    ...
    osf.info(4) << "idx=" << idx << ", mjds=" << *mjds << ", start=" << start
                << ", res=" << res << ", j=" << j << "\n";
  ...
  double begin)
{
  ...
  //    Calls doProfiled to compute the attitude (TAtt) based on the profile
  //    definition Then we compute the attitude during the slew from ira,idec to
  //    the profile (in progress) since the spacecraft won't instantaneously
  //    jump to the profile initial point. It takes some time to slew and the
  //    pointing is constantly changing during the slew The attitude during the
  //    slew is stored in OAtt then the profile attitude is copied from TAtt
  //    into OAtt after the slew ends With much error checking to obscure the
  //    logic
  ...
  if (epoch <= start)
  {
    ...
    // np is the number of profile cycles between the profile epoch and the
    // start time startT is adjusted profile start, so we don't have to generate
    // a bunch of unneeded profile cycles
  ...
  else
  {
    ...
    osf.warn(1) << "###########################################################"
                   "#########################################\n\n";
    osf.warn(1) << "                                      WARNING\n";
    osf.warn(1) << "\tEpoch for Profile Survey (" << epoch
                << ") starting at MJD=" << start << "\n";
    osf.warn(1)
      << "\tis later than the start time. This is not a realistic scenario.\n";
    osf.warn(1)
      << "\tHowever, orbit simulator will try to interpolate back in time\n\n";
    osf.warn(1) << "###########################################################"
                   "#########################################\n\n";
    //  Adjust the epoch time back so the entire interval of interest will have
    //  profile data
  ...
  double nend = end + res;
  osf.info(3) << "\nCalling  doProfiled with startT=" << startT
              << ", while start=" << start << ", begin=" << begin << "\n";
  ...
  osf.info(3) << "Calling getEndPoint  with startT=" << startT
              << ", while start=" << start << ", begin=" << begin << "\n";
  ...
  if (mjds > end)
  {
    ...
    osf.err() << "\n###########################################################"
                 "#########################################\n\n";
    osf.err() << "                                           ERROR\n";
    osf.err() << "\tEnd of slew time (" << mjds
              << ") is greater than the end of this snapshot (" << end << ")\n";
    ...
    /*
       ...
       "\n####################################################################################################\n\n";
        eBufT << "                                            ERROR\n";
        eBufT << "\tEnd of slew time (" << mjds << ") is greater than the end of
       this snapshot (" << end << ")\n"; eBufT << "\tThis might cause a seg
       fault. Please check. Exiting for now...........\n\n"; eBufT <<
       "####################################################################################################\n\n"
       <<std::ends;
  ...
  // MJM does mjds mean end_slew_time? YES, it does (a plea for meaningful
  // variable names)

  // MJM Now, DoSlew puts the initial slew maneuver into OAtt

  // MJM (We don't instantly arrive at the profile start point from the initial
  // position) MJM Sun avoidance during the slew is not modelled (?) MJM The
  // quirkly acceleration limits are included in the overall slew duration
  ...
  //      Would TAtt and OAtt may not be parallel, since TAtt is computed
  //      beginning startT and OAtt starts at the actual start time

  // MJM  k is the index into OAtt of the end of the slew
  // MJM  this loop adjusts idx to point to the time at the end of the slew in
  // TAtt MJM  Again better variable names OAtt_idx and TAtt_idx
  ...
  for (j = 0; j < inum; j++)
  {
    if (fabs(TAtt->mjd[j] - mjds) < res / 3.0)
    {
      ...
      osf.info(4) << "at j-1=" << j - 1 << ", mjd=" << TAtt->mjd[j - 1]
                  << ", while mjds=" << mjds << "\n";
      osf.info(4) << "at j=" << j << ", mjd=" << TAtt->mjd[j]
                  << ", while mjds=" << mjds << "\n";
      idx = j;
      break;
    }
  ...
  //      A gap is a problem only if the points differ by more than the
  //      resolution
  //  if(tdif > 0.0){
  ...
  if (tdif > restol)
  {
    osf.err() << "#############################################################"
                 "#######################################\n\n";
    osf.err() << "                                      ERROR\n";
    osf.err() << "In MakeProfiled, while calculating attitude for observation "
                 "starting at "
              << start << "\n";
    osf.err() << "when merging the calculated attitude for this segment with "
    ...
    osf.err() << "the overall attitude would start at " << OAtt->mjd[k - 1]
              << "\n";
    osf.err() << "while the newly calculated part would start at "
              << TAtt->mjd[idx - 1] << ".\n\n";
  ...
  Attitude* OAtt)
{
  ...
  osf.info(2) << "\n\nStart=" << start << "\nend=" << end
              ...
              << "\nResolution=" << res << "\n";
  osf.info(2) << "Supposed to calculate Profiled Survey from " << start
              << " to " << end << "\n";
  ...
  for (jc = 0; jc <= ncycles + 1; jc++)
  {
    if (jc > 0)
    {
      ...
      {
        tms[16] =
          Ltime + (Lincr / fabs(Lincr)) * (double)((int)(res * secInDay));
    ...
    for (ic = 0; ic <= sz - 1; ic++)
    {
      ...
      osf.info(4) << "jc=" << jc << ", jc1=" << jc1 << ", ic=" << ic
                  << ", ic1=" << ic1 << ", epoch1=" << epoch1
                  << ", epoch2=" << epoch2
                  << ", diff=" << (epoch2 - epoch1) * secInDay
                  << ", angsep= " << angsep << ", slewr=" << slewr << "\n";

      if (fabs(slewr) > SLEW_RATE)
      {
        osf.warn(3) << "WARNING: In doProfiled, going from offset " << ofst[ic]
                    << " to " << ofst[ic + 1] << ",\nwill make the slew rate ("
                    << slewr << ") greater than the nominal maximum value ("
                    << SLEW_RATE << ")\n";
      }
      ...
      for (i = 0; i < ino; i++)
      {
        ...
        osf.info(4) << "mjd=" << mjd << ", end=" << end << ", jc=" << jc
                    << ", ic=" << ic << ", i=" << i << ", ino=" << ino
                    << ", calling MakeSurvey with offset=" << offset << "\n";
...
double
InterpProfile(double* tms, double* ofst, double res)
{
  ...
  for (i = 1; i < 16; i++)
  {
    ...
    if (drem != 0)
    {
      ...
      Lofst[i] =
        (ofst[i - 1] - ofst[i]) * (Ltms[i] - tms[i]) / (tms[i - 1] - tms[i])
        + ofst[i];
    }
    else
    {
...
void
occult(EphemData* EphemPtr,
  double StartTime,
  double EndTime,
  double Resolution,
  Attitude* att,
  double EAA,
  double start_ELT_OFF,
  double stop_ELT_OFF)
{
  ...
  for (i = ist; i < inum; ++i)
  {
    ...
    atEarthOccult(
      vSat, y50, vSun, &eflag, &elvy); // CKS: changed from atEarthOccult2

    if (att->in_occ[j] < -999)
    {
      att->in_occ[j] = 0; // This must be coming from slew, hence occultation is
...
void
doLimbTrace(EphemData* EphemPtr,
  double StartTime,
  double EndTime,
  double Resolution,
  Attitude* att)
{
  ...
  // Populate the staocc and endocc arrays when the attitude index at each
  // target occult entry and exit event
  ...
  else
  {
    for (i = 0; i < ns; i++)
    {
      osf.warn(1) << "Target occulted at ephem index, " << staocc[i]
                  << ", from " << EphemPtr->MJD[staocc[i]] << " to "
                  << EphemPtr->MJD[endocc[i]] << "\n";
    }
  }
